...
def main():
    ...
    # create model
    pretrain_dict_param = torch.load(args.pretrained)
    print("=> using pre-trained model '{}'".format(pretrain_dict_param['arch']))
    model = ELGANet().cuda()
    model.load_state_dict(pretrain_dict_param['state_dict'])
    model.eval()
    args.arch = pretrain_dict_param['arch']
