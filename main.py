...
parser = argparse.ArgumentParser(description='PyTorch ELGANet Training on BSDS500',
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='ELGANet',  help='model architecture')
parser.add_argument('--data', metavar='DIR',default='', help='path to input dataset')
parser.add_argument('--savepath',default='./save_path/', help='path to save ckpt')
...
parser.add_argument('--epochs', default=30000000, type=int, metavar='N', help='number of total epoches, make it big enough to follow the iteration maxmium')
...
parser.add_argument('--lr', '--learning-rate', default=0.0008, type=float,metavar='LR', help='initial learning rate')
...
parser.add_argument('--milestones', default=[45000], metavar='N', nargs='*', help='epochs at which learning rate is divided by 2')
...
def main():
    ...
    new_param_group = {'params': [model.P2SP.para]}
    ...
    if args.pretrained:
        pretrained_dict_param = torch.load(args.pretrained)
        model.load_state_dict(pretrained_dict_param['state_dict'])
        optimizer.load_state_dict(pretrained_dict_param['optimizer'])
        best_EPE = pretrained_dict_param['best_EPE']
        args.start_epoch = pretrained_dict_param['epoch']
